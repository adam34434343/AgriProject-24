...
def Commit():
	...
	sql="insert into raindata values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
...
def View():
	lid=t1.get()
	val=(lid,)
	sql="select *from raindata where lid=%s"
	mycursor.execute(sql,val)
...
def Edit():
	...
	sql="update raindata set loc=%s,vlg=%s,tlk=%s,dst=%s,sta=%s,pln=%s,aar=%s,yer=%s,rfq=%s,stq=%s,qty=%s,abt=%s where lid=%s"
	mycursor.execute(sql,val)
	mydb.commit()
...
root.title("Rain Fall - Information")
root.config(bg='lightblue')

hlab = tk.Label(root, text ="RAIN FALL INFORMATION")
hlab.place(x = 80, y = 10) 
...
y = (screen_height/2) - (h/2)
root.geometry('%dx%d+%d+%d' % (w, h, x, y))
root.title("Rain Fall")
root.config(bg='lightblue')

lab = tk.Label(root, text ="Locality / Rain Fall Information")
lab.place(x=20, y=70) 
