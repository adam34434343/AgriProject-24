...
def Commit():
	...
	sql="insert into fertilizer values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
...
def View():
	lid=t1.get()
	val=(lid,)
	sql="select *from fertilizer where lid=%s"
	mycursor.execute(sql,val)
...
def Edit():
	...
	sql="update fertilizer set loc=%s,vlg=%s,tlk=%s,dst=%s,sta=%s,pln=%s,aar=%s,yer=%s,alt=%s,fna=%s,apl=%s,abt=%s where lid=%s"
	mycursor.execute(sql,val)
	mydb.commit()
...
def Delete():
	lid=t1.get()
	val=(lid,)
	sql="delete from fertilizer where lid=%s"
	mycursor.execute(sql,val)
	mydb.commit()
...
root.title("Fertilizer - Information")
root.config(bg='lightblue')

hlab = tk.Label(root, text ="FERTILIZER ALLOTMENT")
hlab.place(x = 90, y = 10) 
...
l1 = tk.Label(root, text ="Fertilizer - Id" )
l1.place(x=20, y =120)
