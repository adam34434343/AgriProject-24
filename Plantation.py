...
def Commit():
	...
	sql="insert into seedplant values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
...
def View():
	lid=t1.get()
	val=(lid,)
	sql="select *from seedplant where sid=%s"
	mycursor.execute(sql,val)
...
def Edit():
	...
	sql="update seedplant set loc=%s,vlg=%s,tlk=%s,dst=%s,sta=%s,pln=%s,aar=%s,yer=%s,alt=%s,sna=%s,yld=%s,abt=%s where sid=%s"
	mycursor.execute(sql,val)
	mydb.commit()
...
def Delete():
	lid=t1.get()
	val=(lid,)
	sql="delete from seedplant where sid=%s"
	mycursor.execute(sql,val)
	mydb.commit()
...
root.title("Seed & Plantation - Information")
root.config(bg='lightblue')

hlab = tk.Label(root, text ="PLANTATION & SEED ALLOCATION")
hlab.place(x = 10, y = 10) 
hlab.config(fg='blue',bg='lightblue',font=('Helvetica bold', 28))
...
lab = tk.Label(root, text ="Locality / Allotment Information")
lab.place(x=20, y=70) 
lab.config(fg='black',bg='lightblue',font=('Helvetica bold', 15,'bold'))

l1 = tk.Label(root, text ="Seed/Plant - Id" )
l1.place(x=20, y =120)
...
l13 = tk.Label(root, text ="About Seed/Plant" )
